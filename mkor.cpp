...
int getColor()
{
	...
	{
		return 1;
	} else if(colorSensor.color == 2 || colorSensor.color == 3)
	{
		return 2;
...
int findMaxColor()
{
	for(int i = 5; i >= 0; i--)
	{
		if(prices[i] != 6 && prices[i] != 4)
			return prices[i];
	}
	return -1;
}
int findPriceByColor(int col)
{
	for(int i = 0; i < 6; i++)
	{
		if(prices[i] == col)
...
void fillPricesC()
{
	for(int i = 0; i < 6; i++)
	{
		pricesC[i+1>5?0:i+1] = findPriceByColor(color[i]);
	}
	for(int i = 0; i < 6; i++)
	{
		positionsC[pricesC[i]] = i;
	}
	//int temp;
	//for (int i = 0; i < 5; i++) {
	//	for (int j = 0; j < 5 - i; j++) {
	//		if (positionsC[j] > positionsC[j + 1]) {
	//			temp = positionsC[j];
	//			positionsC[j] = positionsC[j + 1];
	//			positionsC[j + 1] = temp;
	//		}
	//	}
	//}
}
void refillTrueColorAdvanced4()
{
	color1C[0] = 4;
	color1C[1] = 6;
	color1C[2] = findMaxColor();
	int c = 0;
	for(int i = 1; i <= 6; i++)
	{
		if(i != 4 && i != 6 && i != color1C[2] && i != color2C[0] && i != color2C[1] && i != color2C[2])
			color2C[c++] = i;
	}
	int temp;
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 2 - i; j++) {
			if (findPriceByColor(color1C[j]) > findPriceByColor(color1C[j + 1])) {
				temp = color1C[j];
				color1C[j] = color1C[j + 1];
				color1C[j + 1] = temp;
			}
		}
	}
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < 2 - i; j++) {
			if (findPriceByColor(color2C[j]) > findPriceByColor(color2C[j + 1])) {
				temp = color2C[j];
				color2C[j] = color2C[j + 1];
				color2C[j + 1] = temp;
			}
		}
	}
...
task main()
{
	...
	setPrices();
	refillTrueColorAdvanced4();
	fillPricesC();
	for(int i = 0; i < 3; i++)
	{
		true_color[positionsC[i]] = color1C[i];
	}
	for(int i = 0; i < 3; i++)
	{
		true_color[positionsC[i+3]] = color2C[i];
	}
	displayCenteredTextLine(5,"%d %d %d %d %d %d",true_color[0], true_color[1], true_color[2], true_color[3], true_color[4], true_color[5]);
	read_color();
	read_true_color();
	true_position();
	graphing();
	...
	movement(find_cargo(positionsC[2],0), positionsC[5]);
	movement(find_cargo(positionsC[1],0), positionsC[4]);
	movement(find_cargo(positionsC[0],0), positionsC[3]);
